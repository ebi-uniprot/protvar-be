...
import static uk.ac.ebi.protvar.model.UserInput.DB_MT_CHROMOSOME;

class UserInputVCFParserTest {
  @ParameterizedTest
  @NullAndEmptySource
  void emptyLineIsInvalidObject(String input) {
    UserInput userInput = UserInput.VCFParser.parse(input);
    assertEquals(UserInput.invalidObject(input), userInput);
  }
  ...
  private void assertInput(boolean valid, String input, String chr, Long pos, String id, String ref, String alt,
    ...
    assertEquals(input, actual.getInputString());
  ...
  class isIdTest {
    ...
    void emptyListWillSayCannotGetUserId() {
      assertFalse(UserInput.VCFParser.isIdExist(new LinkedList<>()));
    }

    @ParameterizedTest
    @ValueSource(strings = {"A", "C", "G", "T"})
    void alleleCannotBeUserIds(String allele) {
      assertFalse(UserInput.VCFParser.isIdExist(new LinkedList<>(List.of(allele))));
    }
