...
public class Pro2Gen {
    ...
    private void convert(List<VariantInput> proteinInputs, TreeMap<String, List<String>> refseqIdMap) {
        ...
        String[] accessions = uniqueAccPos.stream().map(AccessionPosition::accession).toArray(String[]::new);
        Integer[] ppositions = uniqueAccPos.stream().map(AccessionPosition::position).toArray(Integer[]::new);

        // 2. get all the relevant db records by accessions and positions
        Map<String, List<GenomeToProteinMapping>> gCoords = mappingRepo.getMappingsByAccPos(accessions, ppositions)
