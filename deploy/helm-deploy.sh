...
# Create UniProt group registry secret if available
if [[ -n ${UNIPROT_GROUP_READ_REGISTRY+x} && -n "$UNIPROT_GROUP_READ_REGISTRY" ]]; then
  kubectl create secret docker-registry uniprot-group-registry-read \
    --docker-server="$CI_REGISTRY" --docker-username="uniprot_group_read_registry" --docker-password="$UNIPROT_GROUP_READ_REGISTRY" \
    -o yaml --dry-run=client | kubectl apply -f -
fi

# Helm Deployment
chmod +x deploy/helm-post-renderer.sh

helm upgrade --install \
  --reset-values \
  --set imageTag="${imageTag:-latest}" \
  -f deploy/variables-"${ENV}".yaml \
